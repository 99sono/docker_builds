...
if ! docker exec "${CONTAINER_NAME}" bash -lc 'export NODE_BIN_DIR="/home/developer/programs/node/bin"; export PATH="$NODE_BIN_DIR:$PATH"; command -v codex && codex --version' >/dev/null 2>&1; then
    echo "❌ 'codex' not found on PATH inside the container after exporting NODE_BIN_DIR."
    exit 1
fi
